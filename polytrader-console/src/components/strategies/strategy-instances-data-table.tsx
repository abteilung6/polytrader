...
interface StrategyInstancesDataTableProps {
  columns: ColumnDef<StrategyResponse>[]
  data: StrategyResponse[]
}

export function StrategyInstancesDataTable({ columns, data }: StrategyInstancesDataTableProps) {
    ...
    <div className="relative flex w-full flex-col gap-4 overflow-auto">
      <div className="flex items-center justify-between gap-2">
        <Input
          ...
          value={(table.getColumn('strategy_id')?.getFilterValue() as string) ?? ''}
          onChange={(event) => table.getColumn('strategy_id')?.setFilterValue(event.target.value)}
          className="max-w-sm"
        />
        <DropdownMenu>
